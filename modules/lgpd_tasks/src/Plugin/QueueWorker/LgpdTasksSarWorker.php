...
namespace Drupal\lgpd_tasks\Plugin\QueueWorker;
 ...
 *
 * @QueueWorker(
 *   id = "lgpd_tasks_process_lgpd_sar",
...
class LgpdTasksSarWorker extends QueueWorkerBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @var \Drupal\lgpd_tasks\Traversal\RightToAccessDisplayTraversal
   */
   ...
   *   The lgpd sars task queue.
   * @param \Drupal\lgpd_fields\EntityTraversalFactory $rta_traversal
   *   The rta traversal service.
  ...
  public function __construct(array $configuration, $plugin_id, array $plugin_definition, EntityTypeManagerInterface $entity_type_manager, UuidInterface $uuid, FieldTypePluginManager $field_type_plugin_manager, QueueInterface $queue, EntityTraversalFactory $rta_traversal, FileSystem $file_system, MessengerInterface $messenger) {
    ...
    $this->taskStorage = $entity_type_manager->getStorage('lgpd_task');
    $this->uuid = $uuid;
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $container->get('queue')->get('lgpd_tasks_process_lgpd_sar'),
      $container->get('lgpd_tasks.rta_traversal'),
   ...
   *
   * @param \Drupal\lgpd_tasks\Entity\TaskInterface $task
   *   The task.
  ...
  protected function initialise(TaskInterface $task, $build_now = FALSE) {
    ...
    if (!\file_prepare_directory($directory, FILE_CREATE_DIRECTORY)) {
      throw new \RuntimeException('lgpd SARs upload directory is not writable.');
    }
    ...
    do {
      ...
      $file = _lgpd_tasks_file_save_data('', $task->getOwner(), "{$directory}/{$uuid}.zip", FILE_EXISTS_ERROR);
    } while (!$file);
  ...
  public function build(TaskInterface $task) {
    ...
    foreach ($csvs as $filename => $data) {
      if (!isset($files[$filename])) {
        ...
        $file = _lgpd_tasks_file_save_data('', $task->getOwner(), "{$directory}/{$filename}.csv", FILE_EXISTS_REPLACE);
