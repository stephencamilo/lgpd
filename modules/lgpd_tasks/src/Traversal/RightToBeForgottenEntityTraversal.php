 ...
 *
 * @package Drupal\lgpd_tasks\Traversal
 */
class RightToBeForgottenEntityTraversal extends EntityTraversal {
   ...
   */
  private function remove(FieldItemListInterface $field, lgpdField $field_config, EntityInterface $entity) {
    try {
      $should_delete = FALSE;
   ...
   *   First element is success boolean, second element is the error message.
   */
  private function anonymize(FieldItemListInterface $field, FieldDefinitionInterface $field_definition, lgpdField $field_config) {
   ...
   */
  private function getAnonymizerId(FieldDefinitionInterface $field_definition, lgpdField $field_config) {
    ...
    if (!$anonymizer) {
      ...
      $anonymizers = [
        'string' => 'lgpd_text_anonymizer',
        'datetime' => 'lgpd_date_anonymizer',
      ];

      $this->moduleHandler->alter('lgpd_type_anonymizers', $anonymizers);
