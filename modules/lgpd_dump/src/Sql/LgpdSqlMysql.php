...
class LgpdSqlMysql extends SqlMysql {
  ...
  public function __construct($dbSpec, $options) {
    ...
    $this->lgpdDumpConfig = \Drupal::config(SettingsForm::lgpd_DUMP_CONF_KEY);
    $this->tablesToAnonymize = $this->lgpdDumpConfig->get('mapping');
    $this->tablesToSkip = \array_keys($this->lgpdDumpConfig->get('empty_tables'));
  }
  ...
  /**
   * Create table renames according to the lgpd config.
   *
  ...
  protected function createRenameCommands(array $tableSelection) {
    ...
    foreach (\array_keys($this->tablesToAnonymize) as $table) {
      ...
      $clone = lgpdSqlDump::lgpd_TABLE_PREFIX . $table;
