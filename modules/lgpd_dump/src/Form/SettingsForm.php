...
use Drupal\lgpd_dump\Service\lgpdDatabaseManager;
...
class SettingsForm extends ConfigFormBase {
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('lgpd_dump.database_manager')
    );
  }
  ...
  public function getFormId() {
    return 'lgpd_dump_settings_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $anonymizationOptions = [
      ...
      '#options' => $plugins,
      '#empty_value' => self::lgpd_DUMP_NO_PLUGIN_KEY,
      '#empty_option' => $this->t('- No -'),
      '#title_display' => 'invisible',
    ];

    $config = $this->config(self::lgpd_DUMP_CONF_KEY);
    ...
    if ($form_state->hasValue('tables')) {
      ...
      foreach ($tables as $table => $row) {
        ...
        foreach ($row['columns'] as $name => $data) {
          if (!empty($data['option']) && $data['option'] !== self::lgpd_DUMP_NO_PLUGIN_KEY) {
            $mapping[$table][$name] = $data['option'];
          }
        }
      }

      $config = $this->configFactory->getEditable(self::lgpd_DUMP_CONF_KEY);
      $config
        ->set('mapping', $mapping)
        ->save();
