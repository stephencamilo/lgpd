...
use Drupal\lgpd_dump\Exception\lgpdDumpAnonymizationException;
use Drupal\lgpd_dump\Form\SettingsForm;
...
class lgpdSqlDump {
  ...
  public function __construct(
    ...
    Connection $database,
    lgpdDatabaseManager $lgpdDatabaseManager,
    AnonymizerFactory $pluginFactory
  ) {
    $this->tablesToAnonymize = $configFactory->get(SettingsForm::lgpd_DUMP_CONF_KEY)->get('mapping');
    $this->tablesToSkip = $configFactory->get(SettingsForm::lgpd_DUMP_CONF_KEY)->get('empty_tables');
    $this->database = $database;
    $this->driver = $this->database->driver();
    $this->databaseManager = $lgpdDatabaseManager;
  ...
  protected function getInstance(array $options = NULL) {
    return lgpdSqlBase::create($options);
  }
  ...
  protected function createCloneQueryString($originalTable) {
    ...
    $clonedTable = self::lgpd_TABLE_PREFIX . $originalTable;
    switch ($this->driver) {
      case 'mysql':
    ...
    throw new SqlException("Unsupported database driver detected, can't clone table $originalTable for lgpd.");
  }
  ...
  protected function createTableClones() {
    ...
    $transaction = $this->database->startTransaction('lgpd_clone_tables');
  ...
  protected function sanitizeData() {
    ...
    foreach ($this->tablesToAnonymize as $table => $anonymizationOptions) {
      ...
      $clonedTable = self::lgpd_TABLE_PREFIX . $table;
      ...
      while ($row = $oldRows->fetchAssoc()) {
        foreach ($anonymizationOptions as $column => $pluginId) {
          ...
          if ($tries > 50) {
            throw new lgpdDumpAnonymizationException("Too many retries for column '$column'.");
          }
  ...
  protected function cleanup() {
    ...
    foreach (\array_keys($this->tablesToAnonymize) as $table) {
      ...
      $this->database->schema()->dropTable($lgpdTable);
    }
    $this->database->popTransaction($transaction->name());
  }
