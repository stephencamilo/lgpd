...
abstract class EntityTraversal implements EntityTraversalInterface {
  ...
  /**
   * Entity storage for lgpd config entities.
   *
   ...
   *
   * @var \Drupal\lgpd_fields\Entity\lgpdField[]
   */
  ...
  public function __construct(EntityTypeManagerInterface $entityTypeManager, EntityFieldManagerInterface $entityFieldManager, EntityInterface $base_entity) {
    ...
    $this->configStorage = $this->entityTypeManager->getStorage('lgpd_fields_config');
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  protected function doTraversalRecursive(EntityInterface $entity, lgpdField $parent_config = NULL, $row_id = NULL) {
    ...
    if ($definition->get('lgpd_entity_traversal_exclude')) {
      return;
    }
   ...
   *   Parent's config.
   */
  abstract protected function processEntity(FieldableEntityInterface $entity, lgpdFieldConfigEntity $config, $row_id, lgpdField $parent_config = NULL);
