...
class lgpdCollector {
  ...
  /**
   * Constructs a lgpdCollector object.
   *
  ...
  /**
   * List fields on entity including their lgpd values.
   *
  ...
  public function listFields(EntityTypeInterface $entity_type, $bundle_id, array $filters) {
    ...
    $lgpd_settings = lgpdFieldConfigEntity::load($entity_type->id());
    ...
    // is set, return an empty array if lgpd is not configured for the entity.
    if ($filters['empty'] && $lgpd_settings == NULL) {
      return $fields;
    }
    ...
    foreach ($field_definitions as $field_id => $field_definition) {
      ...
      $route_name = 'lgpd_fields.edit_field';
      $route_params = [
