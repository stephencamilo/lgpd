...
/**
 * Plugin implementation of the 'lgpd_consent_widget' widget.
 *
 * Provides the ability to attach a consent agreement to a form.
 *
 * @FieldWidget(
 *   id = "lgpd_consent_widget",
 *   label = @Translation("lgpd Consent"),
 *   description = @Translation("lgpd Consent"),
 *   field_types = {
 *     "lgpd_user_consent",
 *   },
 * )
...
class ConsentWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  public static function create(
    ...
    return new static(
      ...
      $container->get('plugin.manager.lgpd_consent_resolver'),
  ...
  public function __construct(
    ...
    ConsentUserResolverPluginManager $lgpdConsentResolverManager,
    AccountInterface $currentUser
  ) {
    parent::__construct($plugin_id, $plugin_definition, $field_definition, $settings, $third_party_settings);
    $this->lgpdConsentResolverManager = $lgpdConsentResolverManager;
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $canEditAnyonesConsent = $this->currentUser->hasPermission('grant lgpd any consent');
    $canEditOwnConsent = $this->currentUser->hasPermission('grant lgpd own consent');
    ...
    $element['agreed'] = [
      ...
      '#attributes' => ['class' => ['lgpd_consent_agreement']],
      '#attached' => [
        'library' => [
          'lgpd_consent/lgpd_consent_display',
        ],
      ],
    ];
    ...
    if (!$agreement->requiresExplicitAcceptance()) {
      ...
      $element['agreed']['#markup'] =
        '<span class="lgpd_consent_implicit">' . $agreement->get('description')->value . '</span>';
  ...
  private function getConsentingUser(FieldItemListInterface $items) {
    ...
    $resolver = $this->lgpdConsentResolverManager->getForEntityType($definition->getTargetEntityTypeId(), $definition->getTargetBundle());
